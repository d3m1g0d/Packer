...
source "vsphere-iso" "windows-server-standard-core" {
  ...
  cd_files = [
    "${path.cwd}/scripts/${var.vm_guest_os_family}/common/cd_files/"
  ]
  cd_content = {
    ...
    "network.ps1" = templatefile("${abspath(path.root)}/data/network.pkrtpl.hcl", {
  ...
  ip_wait_address   = "${var.vm_ip_address}/${var.vm_ip_netmask}"
